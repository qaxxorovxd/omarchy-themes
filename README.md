...
Nine Omarchy themes, one per colour, each carrying its own set of 18 anime
wallpapers. The wallpapers are not decoration bolted on afterwards — every
theme's palette and its backgrounds were chosen against the same hue, so the
desktop, the terminal and the picture behind them read as one piece.

162 wallpapers in total. All dark. All landscape, 1920px wide at the least,
most of them 4K.
...
Every theme is installed as `影-<slug>`, so all nine sort together at the end
...
./install.sh sakura-drift          # just one (the mark is optional)
...
## How the wallpapers were matched
...
├── backgrounds/       18 graded wallpapers
